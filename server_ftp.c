...
void ftpConnectionHandler(int newsockfd)
{
    ...
    int i, c, req, msg_ok, fail;
    int fileSize;
    int numBlocks, numBlocksTemp, lastNumBlocks, lastNumBlocksTemp, temp;
    int notReadable;
    ...
    if((readIn(newsockfd,(char *)&req,sizeof(req))) < 0)
    {
    ...
    if (req != 100)
    {
        ...
        msg_ok = 160;
        msg_ok = htons(msg_ok);
        if((writeIn(newsockfd,(char *)&msg_ok,sizeof(msg_ok))) < 0)
        {
    ...
    // Done Know if this If is redundent.
    ...
    temp = htons(fail);
    if((writeIn(newsockfd,(char *)&temp,sizeof(temp))) < 0)
    {x
        error("Server: Write Error");
    }
    if(fail == 200)
    ...
    if((writeIn(newsockfd, (char *) &numBlocksTemp, sizeof(numBlocksTemp))) < 0)
    {
    ...
    if((writeIn(newsockfd, (char *) &numBlocksTemp, sizeof(numBlocksTemp))) < 0)
    {
        error("Server: Writing Error At End of File");
    }*/
    ...
    for (i = 0; i < numBlocks; i++)
    {
        ...
        if (lastNumBlocks > 0)
        {
            ...
            if ((writeIn(newsockfd, ftp_buffer, lastNumBlocks)) < 0)
            {
                error("Server: Unable to Transfer File");
            }*/
        }
    }
...
void connectionHandler(int sock)
{
    ...
    // Reading in the Sockets
    
    socketReader = read(sock, buffer, 255);
    
    // Check if Their Was An Issue with Reading
    ...
    // Same Code from Lines 129->131
    
    printf("Here is the message: %s\n",buffer);
    socketReader = write(sock, "I got your message",18);

    // Error Handling for Writing to the Socket
